...
describe("read-only API routes", () => {
  it("returns validated summary, hypotheses and explorer responses for GET", async () => {
    ...
    expect(await summary.json()).toMatchObject({ kind: "summary", filters: { source: "vk_ads" } });
    ...
    expect(await hypotheses.json()).toMatchObject({ kind: "hypotheses", items: [] });
    ...
    expect(await explorer.json()).toMatchObject({ kind: "explorer", tree: [] });
  });
