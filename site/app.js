...
function filtered(items) {
  ...
  return items.filter((item) => {
    ...
    const haystack = `${item.id} ${item.name} ${item.campaign_name ?? ""} ${item.group_name ?? ""} ${lineage}`.toLowerCase();
...
function groupRecord(item, nested = false) {
  ...
  const summary = nested
    ? `<summary><span class="id">#${escapeHtml(item.id)}</span><span class="record-title">${escapeHtml(item.name)}</span>${statusBadge(item)}<span class="descendant-count">${adCount} объявлений</span></summary>`
    : `<summary><span class="id">#${escapeHtml(item.id)}</span><span class="record-title">${escapeHtml(item.name)}</span>${statusBadge(item)}<span class="metric"><small>Бюджет</small><b>${escapeHtml(item.budget)}</b></span>${decisionBadge(item)}</summary>`;
...
function campaignRecord(item) {
  ...
  return `<details class="record campaign-record"><summary><span class="id">#${escapeHtml(item.id)}</span><span class="record-title">${escapeHtml(item.name)}</span>${statusBadge(item)}<span class="metric"><small>Расход</small><b>${escapeHtml(item.spend)}</b></span>${decisionBadge(item)}</summary><div class="record-body"><div class="fact-grid">${body}</div><p class="rationale"><strong>Решение:</strong> ${escapeHtml(item.rationale)}</p><section class="descendants"><div class="descendant-head"><div><span>Уровень 2</span><h3>Группы кампании</h3></div><b>${groups.length}</b></div><div class="child-groups">${groups.map((group) => groupRecord(group, true)).join("")}</div></section></div></details>`;
}

function creativeCard(item) {
  return `<article class="creative-card"><div class="creative-media">${mediaButton(item)}</div><div class="creative-copy"><div class="creative-topline"><span class="id">creative_id ${escapeHtml(item.creative_id)}</span>${decisionBadge(item)}</div><h2>${escapeHtml(item.name)}</h2><p class="headline">${escapeHtml(item.title)}</p><div class="creative-stats"><div><small>Расход</small><b>${escapeHtml(item.spend)}</b></div><div><small>CTR</small><b>${escapeHtml(item.ctr)}</b></div><div><small>CPC</small><b>${escapeHtml(item.cpc)}</b></div><div><small>CPA билета</small><b>${escapeHtml(item.ticket_cpa)}</b></div></div><div class="creative-meta"><span><b>CTA</b>${escapeHtml(item.cta)}</span><span><b>Статус</b>${escapeHtml(item.status)} · ${escapeHtml(item.moderation)}</span><span><b>Текст</b>${escapeHtml(item.short_text)}</span><span class="url"><b>Ссылка</b>${escapeHtml(item.url)}</span><span><b>UTM</b>${item.utm_complete ? "полная" : "неполная"}</span><span><b>Purchase / revenue</b>not_observed</span></div><p class="rationale">${escapeHtml(item.rationale)}</p></div></article>`;
}
